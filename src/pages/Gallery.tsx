...
const Gallery: React.FC = () => {
  ...
  return (
    <div className="bg-green-50 min-h-screen py-8 border-4 border-black rounded-2xl shadow-neobrutal">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-12 animate-fade-in-up">
          <h1 className="font-poppins font-extrabold text-4xl md:text-5xl text-earth-800 mb-4 border-4 border-black inline-block px-6 py-2 rounded-lg shadow-neobrutal">
            Travel Gallery
          </h1>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8 animate-fade-in-up">
          {categories.map((category) => (
            <button
              ...
              className={`px-6 py-3 rounded-full font-roboto font-medium transition-all duration-300 border-4 border-black
            ${activeCategory === category.id
                  ? 'bg-earth-600 text-white shadow-neobrutal'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredPhotos.map((photo, index) => (
            <div
              key={photo.id}
              className="group relative overflow-hidden rounded-lg border-4 border-black shadow-neobrutal hover:shadow-neobrutal-hover transition-all duration-300 transform hover:-translate-y-2 cursor-pointer animate-fade-in-up"
        ...
        <div className="mt-20 animate-fade-in-up">
          <div className="text-center mb-12">
            <h2 className="font-poppins font-extrabold text-3xl text-earth-800 mb-4 border-4 border-black inline-block px-6 py-2 rounded-lg shadow-neobrutal">
              Travel Stories
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((story, index) => (
              <div
                key={story}
                className="bg-white rounded-xl border-4 border-black shadow-neobrutal overflow-hidden hover:shadow-neobrutal-hover transition-all duration-300 transform hover:-translate-y-2 animate-fade-in-up"
      ...
      {/* Lightbox */}
      {selectedImage !== null && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center border-4 border-black shadow-neobrutal">
          <div className="relative max-w-4xl max-h-full p-4">
            {/* Close Button */}
            <button
              onClick={closeLightbox}
              className="absolute top-4 right-4 bg-white/20 backdrop-blur-sm text-white p-2 rounded-full hover:bg-white/30 transition-colors z-10 border-4 border-black shadow-neobrutal"
            >
              <X className="h-6 w-6" />
            </button>
            ...
            <button
              onClick={prevImage}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/20 backdrop-blur-sm text-white p-2 rounded-full hover:bg-white/30 transition-colors border-4 border-black shadow-neobrutal"
            >
            ...
            <button
              onClick={nextImage}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/20 backdrop-blur-sm text-white p-2 rounded-full hover:bg-white/30 transition-colors border-4 border-black shadow-neobrutal"
            >
            ...
            <img
              ...
              className="max-w-full max-h-full object-contain border-4 border-black shadow-neobrutal"
            />

            {/* Image Info */}
            <div className="absolute bottom-4 left-4 right-4 bg-black/50 backdrop-blur-sm text-white p-4 rounded-lg border-4 border-black shadow-neobrutal">
